...
const [allItems, setAllItems] = useState([]);
  ...
  try {
   ...
   const items = (response.data || []).map(item => ({
    ...
    console.log('Mapped food items:', items);
...
const handleRemove = (itemId) => {
  ...
  <input type="text" className="search-bar" placeholder="Search..." onChange={handleSearchChange}/>
 <img src="/search.png" alt="Search" className="search-icon" />

</div>

      <div className="category-buttons">
        {['Burger', 'Pizza', 'Drink', 'French Fries', 'Veggies'].map((c, idx) => (
          <>
      ...
      <div className="items-grid">
        {filteredItems.map(item => (
          <ItemCard key={item._id} item={item} cart={cart} onAdd={handleAdd} onRemove={handleRemove} />
        ))}
      </div>
